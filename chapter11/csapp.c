    ...
    while (nleft > 0) {
        if ((nwritten = write(fd, bufp, nleft)) <= 0) {
            if (errno == EINTR) {
                ...
                // another end is not request anymore and close the connection,
                // so we pretend that we finished the write by `return n` directly
                return n;
            } else {
...
static ssize_t rio_read_before_request_body(rio_t *rp, char *usrbuf, size_t n) {
    int cnt;

    while (rp->rio_cnt <= 0) {  /* Refill if buf is empty */
        int c = 0;
        do {
            ssize_t r = read(rp->rio_fd, rp->rio_buf + c, 1);
            if (r < 0) {
                if (errno != EINTR) /* Interrupted by sig handler return */
                    return -1;
            } else if (r == 0)  /* EOF */
...
ssize_t rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen) {
    ...
    for (n = 1; n < maxlen; n++) {
        if ((rc = rio_read_before_request_body(rp, &c, 1)) == 1) {
            *bufp++ = c;
            if (c == '\n') {
                n++;
                break;
